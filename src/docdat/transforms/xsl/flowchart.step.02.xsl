...
<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg" xmlns:exsl="http://exslt.org/common" match="/">
        <xsl:apply-templates select="project/workbreakdownstructures"/>
    </xsl:template>
<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg" xmlns:exsl="http://exslt.org/common" match="project/workbreakdownstructures">
        ...
        <xsl:element name="svg">
            ...
            <xsl:call-template name="showConnectors"/>
...
<xsl:variable xmlns:xsl="http://www.w3.org/1999/XSL/Transform" name="isVertical">
        <xsl:choose>
            <xsl:when test="//orgchart[@id='0' and @orientation='vertical']">
...
<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg" xmlns:exsl="http://exslt.org/common" name="confirmId">
        <xsl:param name="id"/>
        <xsl:variable name="cnt" select="count(/project/workbreakdownstructures//*[@id=$id])"/>
        <xsl:choose>
            <xsl:when test="$cnt != 1">
                <xsl:value-of select="$id"/>-
...
<!-- right child -->
                ...
                <xsl:if test="parent::orgchart[@id]">
...
<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg" xmlns:exsl="http://exslt.org/common" name="showPointerSymbolById">
        <xsl:param name="id"/>
        <xsl:for-each select="/project/workbreakdownstructures//*[@id=$id]">
            <xsl:call-template name="showPointerSymbol">
...
<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg" xmlns:exsl="http://exslt.org/common" name="showRect">
        ...
        <xsl:param name="stroke" select="'#000000'"/>
        <xsl:param name="stroke-width" select="1"/>
        ...
        <xsl:element name="rect">
            ...
            <xsl:attribute name="style">
                <xsl:value-of select="concat('fill:' ,  $fill , ';stroke-width:'  ,  $stroke-width  ,  ';fill-opacity:'   ,  $fill-opacity ,  ';stroke:' , $stroke)"/>
            </xsl:attribute>

            <!-- xsl:attribute name="style">fill:
                <xsl:value-of select="$fill"/>;stroke-width:
                <xsl:value-of select="$stroke-width"/>;fill-opacity:
                <xsl:value-of select="$fill-opacity"/>;stroke:
                <xsl:value-of select="$stroke"/>
            </xsl:attribute -->
        </xsl:element>
...
<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg" xmlns:exsl="http://exslt.org/common" name="showSymbols">
  ...
  <!-- xsl:param name="maxwidth"/ -->
        ...
        <xsl:if test="name()='orgchart' ">
            <!-- show the rectangular portion of the symbol -->
            ...
            <xsl:call-template name="showText">
                ...
                <xsl:with-param name="txt" select="count(*)"/>
            </xsl:call-template>

            <!--
            pick the flow chart symbol
            -->
            <xsl:choose>
                <!-- show the start symbol -->
                <xsl:when test="@id='0'">
                    <xsl:call-template name="showCircle">
                        <xsl:with-param name="cx" select="$cx"/>
                        <xsl:with-param name="cy" select="$cy + $half-height"/>
                        <xsl:with-param name="radius" select="10"/>
                        <xsl:with-param name="fill" select="'#009900'"/>
                        <xsl:with-param name="fill-opacity" select="0.5"/>
                    </xsl:call-template>

                </xsl:when>
                <!-- 
                show the conditional symbol
                -->
                <xsl:when test="@symbol-type='condition'">
                    <xsl:call-template name="showDiamond">
                        <xsl:with-param name="cx" select="$cx"/>
                        <xsl:with-param name="cy" select="$cy + $half-height"/>
                        <xsl:with-param name="radius" select="10"/>
                        <xsl:with-param name="fill-opacity" select="0.5"/>
                ...
                <!--
                   show the terminal symbol
                   no next sibling
                   -->
                <!-- xsl:when test="count(*)=1" -->
                <xsl:when test="count(following-sibling::*)=0 and count(*)=1">
                    <xsl:call-template name="showCircle">
                        <xsl:with-param name="cx" select="$cx"/>
                        <xsl:with-param name="cy" select="$cy + $half-height"/>
                        <xsl:with-param name="radius" select="10"/>
                        <xsl:with-param name="fill" select="'#990000'"/>
                        <xsl:with-param name="fill-opacity" select="0.5"/>
                    </xsl:call-template>
                </xsl:when>
                <!--
                ...
                <!-- xsl:when test="count(child::*)!=0">
                 ...
                 <!-- 
                 show the task symbol
                 -->
                <xsl:otherwise>
                    <xsl:call-template name="showRect">
                        <xsl:with-param name="x" select="$cx"/>
                        <xsl:with-param name="y" select="$cy + $half-height"/>
                        <xsl:with-param name="w" select="20"/>
                        <xsl:with-param name="h" select="20"/>
